<?php
	...
	if($stmt->rowCount())
	{
		...
		$data1 = [
							...
							if($stmt2->rowCount()){
								...
								foreach($stmt2 as $Machinery){
									...
									if($stmt3->rowCount()){
										...
										$ownr['phone'] = $Owner['phone'];
										$ownr['lat'] = $Owner['gis_location_lat'];
										$ownr['lng'] = $Owner['gis_location_lng'];
				...
				if($count == 0){
					$response["success"] = 0;
					$response["message"] = "No machine available for the specified time in your area.";
				}
