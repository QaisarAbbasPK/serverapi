	...
	if($stmt1->rowCount())
	{
		...
		if($stmt->rowCount())
		{
			...
			foreach($stmt as $reservation){
				...
				$Reservation['area_requested'] = (floatval($reservation['area_requested'])/4046.86);
				$Reservation['data_link'] = $reservation['data_link'];
				$Reservation['request_date'] = $reservation['request_date'];
				$Reservation['actual_area'] = (floatval($reservation['actual_area'])/4046.86);;
				...
				if (!empty($stmt1)) {
					foreach($stmt1 as $farmer) {
						$Reservation["FarmerName"] =  $farmer['name']." ".$farmer['fname'];
					}
				}
				/////////////
				...
				if (!empty($stmt2)) {
					foreach($stmt2 as $machinery) {
						...
						if( $Reservation["machineType"] == $type ){
							$jobsCompleted++;
							$totalCompletedArea += 	floatval($Reservation["actual_area"]);
						}
			...
			$response["totalCompletedArea"] = $totalCompletedArea;
			$response["jobsCompleted"] = $jobsCompleted;
