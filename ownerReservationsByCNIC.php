...
$flat=0;
$flng=0;
if (isset($_GET['cnic']) && isset($_GET['type']) ) {
    $ownerCNIC      = $_GET['cnic'];
    $type      = $_GET['type'];
	...
	if ($stmt1->rowCount()){
		...
		$status = "pending";
		$sql = 'SELECT * FROM ReservationRequest where owner_id= :ownerID ORDER BY rid DESC';
		$stmt = $conn->prepare($sql);
		$stmt->bindParam(':ownerID', $ownerID, PDO::PARAM_STR);
		$stmt->execute();
		if ($stmt->rowCount()) {
			
			$acresRemaining=0;$jobsInQueue=0;
			...
			foreach($stmt as $row) {
				...
				$reservation["area_requested"] = (floatval($row["area_requested"])/4046.86);
				...
				if (!empty($stmt1)) {
					foreach($stmt1 as $farmer) {
						...
						$reservation["gis_lat"] = $farmer['gis_location_lat'];
						$reservation["gis_lng"] = $farmer['gis_location_lng'];
				...
				if (!empty($stmt2)) {
					foreach($stmt2 as $machinery) {
						...
						if( $reservation["machineType"] == $type ){
							$jobsInQueue++;
							$acresRemaining += 	floatval($reservation["area_requested"]);
						}
			...
			$response["totalPendingArea"] = $acresRemaining;
			$response["TotalPending Requests"] = $jobsInQueue;
			// success\
		...
		// no products found
		$response["success"] = 0;
		$response["message"] = "Currently, no reservation requests for Service Provider";
		// echo no users JSON
		echo json_encode($response);
		}
	}
